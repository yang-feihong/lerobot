...
--policy.training_rtc_config.simulated_delay=5 \
--policy.training_rtc_config.delay_distribution=exponential
```

The feature is disabled by default. `simulated_delay` is an **exclusive** upper bound: `5` samples prefix lengths `0, 1, 2, 3, 4`. At 50 Hz, these correspond to 0–80 ms. `exponential` samples with probability proportional to `exp(-d)`, matching the Kinetix reference. `uniform` gives equal probability to every supported delay, matching Kai0's default. Choose a range that covers measured deployment delays while leaving an action suffix to predict.
...
./train_vla_pi05.sh \
  ...
  --training-rtc=true \
  --training-rtc-simulated-delay=5 \
...
The PI0.5 VLA server accepts:

| Flag | Behavior |
| --- | --- |
| `--rtc-mode=inference` | Existing inference-time RTC; the default |
| `--rtc-mode=training` | Learned action-prefix conditioning |
| `--rtc-mode=off` | Ordinary sampling without RTC conditioning |
