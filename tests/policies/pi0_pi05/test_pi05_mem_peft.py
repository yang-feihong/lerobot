...
def test_mem_checkpoint_path_survives_moving_between_hosts(tmp_path, monkeypatch):
    moved_root = tmp_path / "mem_vit_distill_outputs"
    moved_checkpoint = moved_root / "mem_vit_distill_20260716_142702" / "mem_vit_distill_step_25000.pt"
    moved_checkpoint.parent.mkdir(parents=True)
    moved_checkpoint.touch()
    recorded_path = Path(
        "/mnt/old-host/data/mem_vit_distill_20260716_142702/mem_vit_distill_step_25000.pt"
    )

    monkeypatch.delenv("LEROBOT_MEM_VIT_CHECKPOINT", raising=False)
    monkeypatch.setenv("LEROBOT_MEM_VIT_SEARCH_ROOTS", str(moved_root))

    assert modeling_pi05._resolve_mem_vit_checkpoint(recorded_path) == moved_checkpoint.resolve()


def test_mem_checkpoint_explicit_override_takes_precedence(tmp_path, monkeypatch):
    override = tmp_path / "renamed_mem_checkpoint.pt"
    override.touch()
    monkeypatch.setenv("LEROBOT_MEM_VIT_CHECKPOINT", str(override))
    monkeypatch.setenv("LEROBOT_MEM_VIT_SEARCH_ROOTS", str(tmp_path / "unused"))

    assert modeling_pi05._resolve_mem_vit_checkpoint("/missing/original.pt") == override


def test_pi05_from_pretrained_does_not_return_an_unloaded_model(tiny_policy_factory, tmp_path):
    config = tiny_policy_factory(mem_vit_enabled=False).config
    with pytest.raises(OSError):
        modeling_pi05.PI05Policy.from_pretrained(tmp_path / "missing", config=config)
...
def test_mem_peft_disk_roundtrip_preserves_updated_output_and_trainability(
    ...
    with torch.no_grad():
        for param in restored_vision.parameters():
            param.add_(1.0)
