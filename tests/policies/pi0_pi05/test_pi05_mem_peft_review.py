"""Behavioral checks for MEM LoRA checkpointing and gradient recomputation."""
...
from lerobot.policies.factory import _validate_embedded_mem_vit_weights
...
def test_checkpoint_resume_matches_next_optimizer_step(
    ...
    save_checkpoint(
        tmp_path,
        1,
        SimpleNamespace(peft=object(), policy=policy.config, save_pretrained=Mock()),
        wrapped,
        optimizer,
        model_state_dict=wrapped.state_dict() if gathered else None,
    )
    saved_config = json.loads((tmp_path / "pretrained_model" / "config.json").read_text())
    assert saved_config["mem_vit_base_weights_embedded"] is True
    assert saved_config["mem_vit_embedded_tensor_count"] > 0
    _validate_embedded_mem_vit_weights(tmp_path / "pretrained_model", policy.config)
